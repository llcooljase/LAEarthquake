...
; and hopefully through these I can explain to you how this model works.                              ;
;
; There are effectively two steps to running this simulation: 1) setting it up, and 2) running the    ;
; simulation. Let me explain them step-by-step.                                                       ;

to setup
  set make-new? "yes" ; make-new? is a variable that tells the sim to keep making new turtles -- this resets it.
  clear-all
  setup-grid
...
to setup-grid
  ...
  ask patches [
    ifelse pcolor = 0 [set blocks 1000][
      set blocks pcolor]
    if pcolor = white [define-blocks]]
end
  
to define-blocks
  if pcolor = orange [set birth 0]
  if pcolor = red [set birth 0]
  if pcolor = green [set birth 0]
  if pcolor = yellow [set birth 0]
end
...
to go
  ...
  tick
  do-plots
  if ticks > (time-for-everyone-to-leave + 10) [ if all? patches [count turtles with [color != 12 and color != 0] = 0] [ stop ]] ; checks for end
end
